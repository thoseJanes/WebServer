...
void onRequest(const HttpRequest* reqptr, HttpResponse* resp, ContextMap& context)
{
    const HttpRequest& req = *reqptr;
  std::cout << "Headers " << http::methodToString(req.getMethod()) << " " << req.getPath() << std::endl;
  if (!benchmark)
  {
  ...
  if (req.getPath() == "/")
  {
    ...
    resp->setHeaderValue("Content-Type", "text/html");
    resp->setHeaderValue("Server", "WebServer");
  ...
  {
    resp->setStatusCode(200);
    resp->setHeaderValue("Content-Type", "image/png");
    resp->setBody(string(favicon, sizeof favicon));
  }
  else if (req.getPath() == "/hello")
  {
    resp->setStatusCode(200);
    resp->setHeaderValue("Content-Type", "text/plain");
    resp->setHeaderValue("Server", "WebServer");
...
int main(int argc, char* argv[])
{
  ...
  EventLoop loop;
  HttpServer server(&loop, "dummy", InetAddress("0.0.0.0", 8081, AF_INET));
